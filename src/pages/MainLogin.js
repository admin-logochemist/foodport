import React from 'react'
import SigninImage from '../images/panda.png'
import { Button } from '@material-ui/core'
import "./signup.css"
import logo from '../images/logologo.png'
import { Link, useHistory } from "react-router-dom";
function MainLogin() {
    function handleClick() {
        history.push("/SignIn");
      }
      function handleClicked() {
        history.push("/USignIn");
      }
      const history = useHistory();
    return (
        <div>
        <div className="login">
          <div className="login__container">
          <img className="logo-png" style={{margin:30}} src={logo}/>
          <Button style={{ backgroundColor: '#d70000', color: 'white',padding:20,margin:10,borderRadius:50,fontWeight:900 }} onClick={() => handleClicked()}>LOGIN AS USER</Button>
          <Button style={{ backgroundColor: '#d70000', color: 'white',padding:20,borderRadius:50,fontWeight:900 }} onClick={() => handleClick()}>LOGIN AS RESTURANT OWNER</Button>
         
            {/* <img src="data:image/png;base64," className="logo" />               */}
       
          </div>
          <div className="sign-in-image">
            <img src={SigninImage} />
          </div>
        </div>
        <div className="copyright-signup-login">
          <h1 style={{ fontSize: 20, }}>Copyright 2020.All rights reserved.</h1>
        </div>
      </div>
    )
}

export default MainLogin
